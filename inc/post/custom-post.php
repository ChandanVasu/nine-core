...
// Class to manage custom meta boxes for the custom post type
class nine_Meta_Boxes {
    ...
    private function __construct() {
        add_filter('manage_nine_theme_posts_columns', [$this, 'add_column']);
        add_action('manage_nine_theme_posts_custom_column', [$this, 'column_data'], 10, 2);
    }

    public function add_column($columns) {
        $columns['nine_theme_template_column'] = __('Nine Theme Template', 'nine-theme');
        return $columns;
    }

    public function column_data($column, $post_id) {
        switch ($column) {
            case 'nine_theme_template_column':
                echo '<input type=\'text\' class=\'widefat\' value=\'[Nine_Theme_Template id="' . $post_id . '"]\' readonly="">';
                break;
        }
    }
}

nine_Meta_Boxes::instance();

// Class to manage the shortcode rendering for the custom post type
class nine_Shortcode {
    ...
    private function __construct() {
        add_shortcode('Nine_Theme_Template', [$this, 'render_shortcode']);
